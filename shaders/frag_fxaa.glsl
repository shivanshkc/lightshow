...
void main() {
    vec2 texelSize = vec2(1./1280., 1./720.);
    ...
    vec3 rgbA = 0.5 * (texture(tex, TexCoord.xy + dir * (1.0 / 3.0 - 0.5)).xyz + 
        texture(tex, TexCoord.xy + dir * (2.0 / 3.0 - 0.5)).xyz);
    vec3 rgbB = rgbA * 0.5 + 0.25 * (texture(tex, TexCoord.xy + dir * -0.5).xyz +
        texture(tex, TexCoord.xy + dir * 0.5).xyz);

    float lumaB = dot(rgbB, luma);
    if ((lumaB < lumaMin) || (lumaB > lumaMax)) {
        FragColor = vec4(rgbA, color.a);
    } else {
        FragColor = vec4(rgbB, color.a);
    }
}
